...
function showlist() {
	...
	var page_data = {"ps" : pagesize, "cp" : currentpage, "bcode" : 401, "zcode" : 1}
	//console.log(gg);
	$.ajax(
			{
				url:"boardList.do",
...
function uppage() {
	myTotal();
	var searchword = $('#searchword').val();
	console.log("서치 워드" + searchword);
	console.log(typeof(searchword));
	console.log(searchword == "");
	var page_data;
	
	if(searchword == ""){
		page_data = {"ps" : pagesize, "cp" : currentpage, "bcode" : 401, "zcode" : 1}
	}else{
		page_data = {"ps" : pagesize, "cp" : currentpage, "bcode" : 401, "zcode" : 1, "sw" : searchword}
	}
		...
		$.ajax(
				{
					...
					error:function(xhr){
						alert(" 업 페이지 비동기 처리 실패~~~~~" + xhr.status + " / " + xhr.statusText);
					}
...
function search() {
	 ...
	 myTotal();// 서치 들어 가면 서치 한 글의 숫자 카운트  될까??
	var searchword = $('#searchword').val();
	console.log(searchword);
	var pagesize = $('#pagesize :selected').val();
	var currentpage = 1;
					...
					<aside class="lg-side">
						...
						<div class="inbox-body">
							<div class="mail-option">
								...
								<div class="btn-group">
									<a
										href="gotoWrite.do?zcode=0&bcode=401&tcode=0&cp=${requestScope.cp}&ps=${requestScope.ps}&idx=0"
												...
												<c:choose>
													...
													<c:otherwise>
														<td class="view-message  text-right"><a
															href="boardDetail.do?bcode=401&edit=1&tcode=0&idx=${blist.idx }&cp=${cp}&ps=${ps}&zcode=0"
															class="btn mini blue">수정</a></td>
														<td class="view-message  text-right"><a
															href="boardDelete.do?bcode=401&tcode=0&idx=${blist.idx }&cp=${cp}&ps=${ps}&zcode=0"
